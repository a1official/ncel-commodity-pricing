...
def search_commodities(q: str = Query(..., min_length=1), db: Session = Depends(get_db)):
    """Search commodities by name with autocomplete support."""
    results = db.query(models.Commodity).filter(models.Commodity.name.ilike(f"%{q}%")).all()
    if not results:
...
def get_prices(
    ...
    if commodity_id: query = query.filter(models.PriceRecord.commodity_id == commodity_id)
    if commodity_name: query = query.filter(models.Commodity.name.ilike(f"%{commodity_name}%"))
    if variety_id: query = query.filter(models.PriceRecord.variety_id == variety_id)
    if variety_name: query = query.filter(models.Variety.name.ilike(f"%{variety_name}%"))
    if market_id: query = query.filter(models.PriceRecord.market_id == market_id)
    if market_name: query = query.filter(models.Market.name.ilike(f"%{market_name}%"))
    if state_name: query = query.filter(models.State.name.ilike(f"%{state_name}%"))
    if source_id: query = query.filter(models.PriceRecord.source_id == source_id)
    if source_name: query = query.filter(models.Source.name.ilike(f"%{source_name}%"))
    if start_date: query = query.filter(models.PriceRecord.date >= start_date)
    if end_date: query = query.filter(models.PriceRecord.date <= end_date)
    if category: query = query.filter(models.Commodity.category.ilike(f"%{category}%"))
...
def get_commodity_prices(
    ...
    if source_filter and source_filter != "All Sources":
        ...
        if source: query = query.filter(models.PriceRecord.source_id == source.id)
    prices = query.order_by(models.PriceRecord.date.asc()).all()
    if not prices: raise HTTPException(status_code=404, detail="No price data found")
    return {
...
def search_markets(q: str = Query(..., min_length=1), db: Session = Depends(get_db)):
    ...
    results = db.query(models.Market).filter(models.Market.name.ilike(f"%{q}%")).limit(50).all()
    return {"query": q, "results": results, "count": len(results)}
...
def get_sources(db: Session = Depends(get_db)):
    ...
    return {
        "sources": sources,
        "count": len(sources),
        "available_filters": ["All Sources", "AGMARKNET", "USDA", "FAO", "APEDA", "MPEDA", "NCDEX", "MCX", "FMPIS"],
    }
...
def get_all_forecasts(db: Session = Depends(get_db), weeks: int = Query(6, ge=1, le=12)):
...
def get_forecast(commodity_id: int, db: Session = Depends(get_db), weeks: int = Query(6, ge=1, le=12)):
    """Get multi-week price forecast for a commodity."""
    commodity = db.query(models.Commodity).filter_by(id=commodity_id).first()
    if not commodity: raise HTTPException(status_code=404, detail="Commodity not found")
    forecaster = MultiSignalForecaster(db)
    forecast = forecaster.get_forecast(commodity_id, weeks)
    if "error" in forecast: raise HTTPException(status_code=400, detail=forecast["error"])
    return forecast
...
def get_daily_average(commodity_id: int, db: Session = Depends(get_db)):
    ...
    avg_price = db.query(func.avg(models.PriceRecord.modal_price)).filter(models.PriceRecord.commodity_id == commodity_id).scalar()
    avg_normalized = db.query(func.avg(models.PriceRecord.normalized_price_per_kg)).filter(models.PriceRecord.commodity_id == commodity_id).scalar()
    if avg_price is None: raise HTTPException(status_code=404, detail="No data found")
    return {
        "commodity_id": commodity_id,
...
def get_price_range(commodity_id: int, db: Session = Depends(get_db), days: int = Query(30, ge=1, le=365)):
    """Get min/max/avg prices for a commodity over a period."""
    start_date = datetime.now().date() - timedelta(days=days)
    records = db.query(models.PriceRecord).filter(models.PriceRecord.commodity_id == commodity_id, models.PriceRecord.date >= start_date).all()
    if not records: raise HTTPException(status_code=404, detail="No data found")
...
def get_source_comparison(commodity_id: int, db: Session = Depends(get_db), days: int = Query(30, ge=1, le=365)):
    ...
    ).join(models.Source).filter(models.PriceRecord.commodity_id == commodity_id, models.PriceRecord.date >= start_date).group_by(models.Source.name).all()
...
async def trigger_ingestion(background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    return {"status": "success", "message": "Ingestion cycle started in background", "timestamp": datetime.now().isoformat()}
...
def trigger_source_ingestion(source_name: str, db: Session = Depends(get_db)):
    ...
    if not connector: raise HTTPException(status_code=404, detail=f"Unknown source: {source_name}")
    try:
        raw_data = connector.fetch_data(datetime.now())
        normalized_data = connector.transform_to_standard(raw_data)
        return {"status": "success", "source": source_name, "raw_records": len(raw_data), "normalized_records": len(normalized_data)}
...
def get_mpeda_export_data(
    ...
    from app.services.mpeda_downloader import load_latest_data, download_mpeda_data, parse_item_wise_data
    try:
        df = load_latest_data("item") or download_mpeda_data("item")
        if df is None: raise HTTPException(status_code=503, detail="Unable to fetch MPEDA data")
        records = parse_item_wise_data(df)
        if commodity: records = [r for r in records if commodity.lower() in r["commodity"].lower()]
        if year: records = [r for r in records if r["year"] == year]
...
def refresh_mpeda_data():
    ...
    if df is None: raise HTTPException(status_code=503, detail="Failed to download MPEDA data")
    records = parse_item_wise_data(df)
...
def get_marine_states(db: Session = Depends(get_db)):
    ...
    states = db.query(distinct(models.State.name))\
        ...
        .order_by(models.State.name).all()
    
    result = [s[0] for s in states]
    if cache.enabled: cache.set_marine_states(result)
    return result
...
def get_marine_summary(db: Session = Depends(get_db)):
    ...
    records = db.query(
     ...
     .filter(models.Commodity.category == "Marine Products")\
     .filter(models.Source.name == "FMPIS")\
     .order_by(models.State.name, models.Commodity.name, models.PriceRecord.date.desc()).all()
    ...
    if cache.enabled: cache.set_marine_summary(result)
    return result
