...
export default function MarineDashboard() {
    ...
    const pricesParams = useMemo(() => ({ category: 'Marine Products', skip: 0, limit: 300 }), []);
    const pricesQuery = useQuery({
        queryKey: queryKeys.prices(pricesParams),
        queryFn: ({ signal }) => fetchPrices(pricesParams, { signal }),
    });

    const marineSummaryQuery = useQuery({
        queryKey: queryKeys.marineSummary,
        queryFn: ({ signal }) => fetchMarineSummary({ signal }),
    });

    const marineStatesQuery = useQuery({
        queryKey: queryKeys.marineStates,
        queryFn: ({ signal }) => fetchMarineStates({ signal }),
    });

    const mpedaParams = useMemo(() => ({ limit: 100 }), []);
    const mpedaQuery = useQuery({
        queryKey: queryKeys.mpedaExportData(mpedaParams),
        queryFn: ({ signal }) => fetchMpedaExportData(mpedaParams, { signal }),
    });

    const loading = commoditiesQuery.isLoading || pricesQuery.isLoading || marineSummaryQuery.isLoading || marineStatesQuery.isLoading;
    const mpedaLoading = mpedaQuery.isLoading;

    const allCommodities = useMemo(() => commoditiesQuery.data ?? [], [commoditiesQuery.data]);
    const commodities = useMemo(() => allCommodities.filter((c: any) => c.category === 'Marine Products'), [allCommodities]);
    const prices = useMemo(() => pricesQuery.data ?? [], [pricesQuery.data]);
    const marineSummary = useMemo(() => marineSummaryQuery.data ?? [], [marineSummaryQuery.data]);
    const marineStates = useMemo(() => marineStatesQuery.data ?? [], [marineStatesQuery.data]);
    const mpedaData = useMemo(() => mpedaQuery.data?.records ?? [], [mpedaQuery.data]);
    const mpedaError = mpedaQuery.isError ? 'Failed to load MPEDA data' : (mpedaLoading || mpedaData.length > 0 ? null : 'No data available');

    useEffect(() => {
        if (loading && !loadTokenRef.current) {
            loadTokenRef.current = markPageLoadStart('marine');
        }
        if (!loading && loadTokenRef.current) {
    ...
    const marineHubs = useMemo(() => {
        const uniqueMarkets: string[] = Array.from(new Set(prices.map((p: any) => String(p.market_name || '')).filter(Boolean)));
        return uniqueMarkets.map((market: string) => {
